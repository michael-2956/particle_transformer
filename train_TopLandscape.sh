...
if [[ "$model" == "ParT" ]]; then
    ...
    optimizer="soap"
    batch_size=2048
elif [[ "$model" == "ParT-Long64-Nano-NoDrop-NoTrim" ]]; then
    modelopts="networks/ParticleTransformerLongNanoNoDropoutNoTrim.py --use-amp --optimizer-option weight_decay 0.01"
    lr="3e-3"
    optimizer="soap"
    batch_size=2048
